...
$patchPath = Join-Path $repoRoot "third_party\paraformer-worker\paraformer-worker.patch"
$targetPath = Join-Path $repoRoot "runtimes\llama-funasr-paraformer.exe"

if ([string]::IsNullOrWhiteSpace($SourceDirectory)) {
    $SourceDirectory = Join-Path $repoRoot ".tmp\FunASRUpstream"
}

if ([string]::IsNullOrWhiteSpace($BuildDirectory)) {
    $driveRoot = [System.IO.Path]::GetPathRoot($repoRoot)
    $BuildDirectory = Join-Path $driveRoot ".vrchatvoiceinput-build\paraformer"
}
...
if (!(Test-Path -LiteralPath $patchPath -PathType Leaf)) {
    throw "Paraformer worker patch was not found: $patchPath"
}
...
if (!$patchAlreadyApplied) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "FunASR source has changes that conflict with the Paraformer worker patch."
    }
...
$expectedSource = "runtime/llama.cpp/paraformer/funasr-paraformer/funasr-paraformer.cpp"
...
Invoke-CheckedCommand -Command "cmake" -Arguments @(
    ...
    "--config", "Release",
    "--target", "llama-funasr-paraformer",
    "--parallel"
)

$builtExecutable = Join-Path $BuildDirectory "bin\Release\llama-funasr-paraformer.exe"
if (!(Test-Path -LiteralPath $builtExecutable -PathType Leaf)) {
    throw "Paraformer build completed without the expected executable: $builtExecutable"
}
...
Write-Host "Paraformer resident runtime installed: $targetPath"
